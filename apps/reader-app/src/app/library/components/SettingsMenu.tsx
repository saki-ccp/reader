...
const SettingsMenu: React.FC<SettingsMenuProps> = ({ onPullLibrary, setIsDropdownOpen }) => {
  ...
  const handleClearBackgroundImage = () => {
    saveSysSettings(envConfig, 'libraryBackgroundImage', '');
    setIsDropdownOpen?.(false);
  };
  ...
  return (
    ...
    >
      ...
      {settings.libraryBackgroundImage && (
        <MenuItem label={_('Clear Background Image')} onClick={handleClearBackgroundImage} />
      )}
      <MenuItem
      ...
      {user && userProfilePlan === 'free' && (
        <MenuItem label={_('Upgrade to Reader Premium')} onClick={handleUpgrade} />
      )}
      {isWebAppPlatform() && <MenuItem label={_('Download Reader')} onClick={downloadReader} />}
      <MenuItem label={_('About Reader')} onClick={showAboutReader} />
      <MenuItem
        label={_('Help improve Reader')}
