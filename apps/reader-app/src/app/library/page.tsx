...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  const backgroundImage = settings.libraryBackgroundImage;
  const bgOpacity = settings.libraryBackgroundOpacity ?? 0.3;
  const bgBlur = settings.libraryBackgroundBlur ?? 0;
  const bgSize = settings.libraryBackgroundSize || 'cover';
  ...
  return (
    <div
      ...
      className={clsx(
        'library-page text-base-content full-height relative flex select-none flex-col overflow-hidden',
        !backgroundImage && (viewSettings?.isEink ? 'bg-base-100' : 'bg-base-200'),
    ...
    >
        ...
        <>
          <div
            ...
            style={{
              ...
              backgroundRepeat: bgSize === 'auto' ? 'repeat' : 'no-repeat',
              filter: bgBlur ? `blur(${bgBlur}px)` : undefined,
              // Prevent blur edge artifacts by slightly expanding the element
              margin: bgBlur ? `-${bgBlur}px` : undefined,
              padding: bgBlur ? `${bgBlur}px` : undefined,
            }}
            aria-hidden='true'
          />
          <div
      ...
      {showBookshelf &&
        (libraryBooks.some((book) => !book.deletedAt) ? (
          ...
          <div className='hero drop-zone relative z-10 h-screen items-center justify-center'>
            <DropIndicator />
