  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `Reader_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `Reader_${data.version}_${arch}-portable.exe`;
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `Reader_${data.version}_${arch}.AppImage`;
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of Reader is available!')}
              </h2>
              <p className='mb-2'>
                {_('Reader {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in Reader")}
