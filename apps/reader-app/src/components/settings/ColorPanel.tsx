...
import LibraryBackgroundSelector from './color/LibraryBackgroundSelector';
...
const ColorPanel: React.FC<SettingsPanelPanelProp> = ({ bookKey, onRegisterReset }) => {
  ...
  const [backgroundBlur, setBackgroundBlur] = useState(viewSettings.backgroundBlur ?? 0);
  ...
  const [libraryBgImage, setLibraryBgImage] = useState(settings.libraryBackgroundImage || '');
  const [libraryBgOpacity, setLibraryBgOpacity] = useState(settings.libraryBackgroundOpacity ?? 0.3);
  const [libraryBgBlur, setLibraryBgBlur] = useState(settings.libraryBackgroundBlur ?? 0);
  const [libraryBgSize, setLibraryBgSize] = useState(settings.libraryBackgroundSize || 'cover');
  const isLibraryMode = !bookKey;
  ...
  useEffect(() => {
    if (backgroundBlur === (viewSettings.backgroundBlur ?? 0)) return;
    saveViewSettings(envConfig, bookKey, 'backgroundBlur', backgroundBlur);
    applyBackgroundTexture();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [backgroundBlur]);
  ...
  const applyBackgroundTexture = () => {
    ...
    document.documentElement.style.setProperty('--bg-texture-blur', `${backgroundBlur}px`);
    document.documentElement.style.setProperty('--bg-texture-size', backgroundSize);
  };

  const handleLibraryBgImageChange = (dataUrl: string) => {
    setLibraryBgImage(dataUrl);
    saveSysSettings(envConfig, 'libraryBackgroundImage', dataUrl);
  };

  const handleLibraryBgOpacityChange = (opacity: number) => {
    setLibraryBgOpacity(opacity);
    saveSysSettings(envConfig, 'libraryBackgroundOpacity', opacity);
  };

  const handleLibraryBgBlurChange = (blur: number) => {
    setLibraryBgBlur(blur);
    saveSysSettings(envConfig, 'libraryBackgroundBlur', blur);
  };

  const handleLibraryBgSizeChange = (size: string) => {
    setLibraryBgSize(size);
    saveSysSettings(envConfig, 'libraryBackgroundSize', size);
  };
    ...
    <div className='my-4 w-full space-y-6'>
      {showCustomThemeEditor ? (
        ...
        <>
          ...
          {isLibraryMode ? (
            <LibraryBackgroundSelector
              backgroundImage={libraryBgImage}
              backgroundOpacity={libraryBgOpacity}
              backgroundBlur={libraryBgBlur}
              backgroundSize={libraryBgSize}
              onImageChange={handleLibraryBgImageChange}
              onOpacityChange={handleLibraryBgOpacityChange}
              onBlurChange={handleLibraryBgBlurChange}
              onSizeChange={handleLibraryBgSizeChange}
            />
          ) : (
            <>
