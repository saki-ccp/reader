...
pub fn run() {
    ...
    let builder = builder.plugin(
        tauri_plugin_single_instance::Builder::new()
            ...
            .dbus_id("com.reader.app".to_owned())
            .build(),
    );
    ...
    builder
        .setup(|#[allow(unused_variables)] app| {
            // Clean up potentially corrupted plugin state files on startup
            #[cfg(desktop)]
            {
                if let Ok(app_data_dir) = app.path().app_data_dir() {
                    // Remove corrupted window-state plugin file if it exists
                    let window_state_file = app_data_dir.join(".window-state.json");
                    if window_state_file.exists() {
                        if let Ok(content) = std::fs::read_to_string(&window_state_file) {
                            if serde_json::from_str::<serde_json::Value>(&content).is_err() {
                                log::warn!("Removing corrupted window state file");
                                let _ = std::fs::remove_file(&window_state_file);
                            }
                        }
                    }
                    // Remove corrupted persisted-scope plugin file
                    let scope_file = app_data_dir.join(".persisted-scope");
                    if scope_file.exists() {
                        if let Ok(content) = std::fs::read_to_string(&scope_file) {
                            if serde_json::from_str::<serde_json::Value>(&content).is_err() {
                                log::warn!("Removing corrupted persisted scope file");
            ...
            // Fallback: ensure the window becomes visible even if window-state plugin fails
            #[cfg(all(not(target_os = "macos"), desktop))]
            {
                let win_clone = win.clone();
                std::thread::spawn(move || {
                    std::thread::sleep(std::time::Duration::from_secs(2));
                    if !win_clone.is_visible().unwrap_or(true) {
                        log::info!("Window not visible after 2s, showing as fallback");
