...
Readest uses three test tiers, all powered by Vitest.
...
- Adds a runtime capability granting plugin permissions to loopback remote URLs (for example, localhost on port 127.0.0.1), so Vitest's browser-mode iframe can call Tauri IPC
